        ...
        for i, inp in enumerate(inputs):
            ...
            while current_node:
                if isinstance(current_node, ReturnNode):
                    return_value = (
                        eval(current_node.code, current_run_ctx, current_run_ctx)
                ...
                if hasattr(current_node, "is_generator") and current_node.is_generator:
                    async for yielded_item_from_node in current_node.execute(
                        current_run_ctx
                    ):
                        yield yielded_item_from_node
                    # After a YieldNode, continue to its next node
                    current_node = current_node.next
                else:
                    # For ActionNode and ConditionNode, execute will be async if they handle await
                    next_node_or_val = await current_node.execute(
                        current_run_ctx
                    )  # Await here
                    if (
                        isinstance(next_node_or_val, tuple)
                        and next_node_or_val[0] == "YIELD"
                    ):
